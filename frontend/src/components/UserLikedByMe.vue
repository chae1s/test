...
<script>
import {unLikeUser, readUserLikedByMe} from '@/api/index.js';

export default {
    name: "LikedByMe",
    async created() {
        try {
            const response = await readUserLikedByMe();
            this.userLikedByMe = response.data;
    ...
    computed: {
        showResultTable() {
            return this.userLikedByMe.length > 0
        },
    },
    methods: {
        async unLikeUser(userId, user) {
            try {
                console.log(`회원 \"${userId}.${user.nickname}\" 즐겨찾기 취소`)
                console.log(`즐겨찾기 취소: isUnLiked ${user.isUnLiked}`)
                const {data} = await unLikeUser(userId);
                user.isUnLiked = true;
                console.log(data);

            } catch (error) {
                if (error.response && error.response.status === 400) {
                    alert("이미 즐겨찾기 취소된 회원입니다.");
                    user.isLiked = true;
                } else {
                    alert(error.response.message)
                }
                console.error("즐겨찾기 취소 오류:", error);
            }
        },
    },
};
